...
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 3, 2, TO_DATE('20/07/02', 'YY/MM/DD'));
...
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 2, 5, TO_DATE('20/07/06', 'YY/MM/DD'));
...
SELECT O.ORDER_ID  AS 주문번호
     , B.BOOK_ID   AS 책번호
     , B.BOOK_NAME AS 책이름
  FROM ORDER_T O LEFT OUTER JOIN BOOK_T B
...
SELECT O.ORDER_ID  AS 구매번호
  ...
  FROM ORDER_T O INNER JOIN CUSTOMER_T C
    ...
    ON O.BOOK_ID = B.BOOK_ID
 WHERE O.ORDER_DT NOT IN (SELECT ORDER_DT
                            FROM ORDER_T
...
SELECT C.CUST_NAME AS 고객명
 ...
 GROUP BY C.CUST_ID, C.CUST_NAME; --고객이름은 중복이 있을 수도 있기 때문에 ID를 추가하여 확실하게 구분한다.
 
-- '김연아'가 구매한 책이 몇 종인가 (피겨의 정석을 2번 주문했다면 구매한 책은 1종)
SELECT C.CUST_NAME AS 고객명
     , COUNT(DISTINCT O.BOOK_ID) AS 구매도서수 -- 몇가지 종류인가
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
...
-- 박지성  3
SELECT C.CUST_NAME AS 고객명
     , COUNT(DISTINCT B.PUBLISHER) AS 출판사수 -- 같은 출판사 다른 책을 구매했을수도 있기 때문에 중복제거를 한다.
  FROM BOOK_T B INNER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C
    ON O.CUST_ID = C.CUST_ID
