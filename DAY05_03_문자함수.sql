...
--실습 
-- 1. 사원 테이블의 JOB_ID에서 밑줄(_) 앞/뒷부분 분리 조회하기
-- EX) IT_PROG  -> IT/PROG
--     글자수   : 7
--     밑줄위치 : 3
--     밑줄 앞 글자수 : 2(밑줄위치 -1)
...
SELECT JOB_ID
     ...
     -- '_' 앞의 글자 반환
     -- JOB_ID칼럼, 1번째 글자부터, INSTR(JOB_ID, '_') -1 : JOB_ID칼럼의 '_' 자릿수를 찾고 -1 후 반환
     
     , SUBSTR(JOB_ID, INSTR(JOB_ID, '_') + 1, LENGTH(JOB_ID) - INSTR(JOB_ID, '_' ))
     -- '_' 뒤의 글자 반환
     -- JOB_ID칼럼, INSTR(JOB_ID, '_') + 1 : JOB_ID칼럼의 '_' 자릿수를 찾고 + 1 후, LENGTH(JOB_ID)총 자릿수 - INSTR(JOB_ID, '_' ) _의 자릿수만큼 반환
     
     , SUBSTR(JOB_ID, INSTR(JOB_ID, '_') + 1)
     -- '_' 뒤의 글자 반환 [반환자릿수]
     -- JOB_ID칼럼, INSTR(JOB_ID, '_') : JOB_ID칼럼의 '-'자릿수를 찾고, + 1 후 일부반환 [반환자릿수]
  FROM EMPLOYEES;

 
 
--2. FIRST_NAME과 LAST_NAME을 연결해서 모두 대문자로 바꾼 FULL_NAME 조회하기
     -- 작동 순서 FIRST_NAME, 공백하나, LAST_NAME과 연결 -> 대문자 변환 -> EMPLOYEE_ID로 오름차순 정렬
