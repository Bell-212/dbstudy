...
SELECT AVG(COMMISSION_PCT) AS 커미션퍼센트평균
  FROM EMPLOYEES
 WHERE COMMISSION_PCT IS NOT NULL; --NULL을 제외하는 조건식이지만 사실 필요하지 않다. AVG 함수는 자체적으로 NULL을 제외한다.
 
--3. 사원 테이블에서 전체 사원중 최대 연봉 조회하기
...
SELECT MAX(SALARY * COMMISSION_PCT) AS 최대커미션
  FROM EMPLOYEES;
--5. 사원 테이블에서 전체 사원의 가장 나중에 입사한 사원의 입사일 조회하기
...
SELECT COUNT(DISTINCT DEPARTMENT_ID) AS 부서수 --DISTINCT는 칼럼 앞에 붙이기
  FROM EMPLOYEES;