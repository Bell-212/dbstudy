...
SELECT USER_NO, USER_ID, USER_REGDATE
  FROM USER_T
 WHERE TO_DATE(USER_REGDATE, 'YY/MM/DD') >= TO_DATE('2010/01/01', 'YY/MM/DD');
    -- EXTRACT(YEAR FROM USER_REGDATE) >= 2010;
    
-- 4. 사용자번호와 연락처1, 연락처2를 연결하여 조회하시오. 연락처가 없는 경우 NULL 대신 'None'으로 조회하시오.
--SELECT USER_ID, NVL(USER_MOBILE1 || USER_MOBILE2, 'None') 
...
SELECT P.PROD_NAME AS 제품명
  ...
  FROM PRODUCT_T P LEFT OUTER JOIN BUY_T B
    ON P.PROD_CODE = B.PROD_CODE
 GROUP BY P.PROD_CODE, P.PROD_NAME; --코드와 이름이 동일해야 하나의 데이터로 본다.
...
-- LHJ     이휘재  2         80
-- KHD     강호동  3         1210
SELECT U.USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(B.BUY_NO) AS 구매횟수, SUM(B.BUY_AMOUNT * P.PROD_PRICE) AS 총구매액
...
SELECT U.USER_NAME AS 고객명, P.PROD_NAME AS 구매제품
  FROM PRODUCT_T P RIGHT OUTER JOIN BUY_T B
    ON P.PROD_CODE = B.PROD_CODE RIGHT OUTER JOIN USER_T U
    ON B.USER_NO = U.USER_NO
 ORDER BY U.USER_ID ASC;
