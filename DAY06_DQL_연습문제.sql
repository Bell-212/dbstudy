...
SELECT *
  FROM EMPLOYEES
 WHERE JOB_ID IN('IT_PROG', 'ST_MAN');  -- 추천
 --WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'ST_MAN';

-- 6. EMPLOYEES 테이블에서 HIRE_DATE가 2005년도인 사원을 조회하시오.
SELECT *
  FROM EMPLOYEES
 WHERE TO_CHAR(HIRE_DATE, 'YYYY') LIKE '05' || '%';
--WHERE HIRE_DATE LIKE '05' || '%' ; 역시 가능하지만 2005년도가 특정되어있기 때문에 HIRE_DATE를 TO_CHAR(문자타입으로 변환)후 4자리 년도 부여 하고 조건식 부여
...
SELECT COMMISSION_PCT * SALARY AS 실제커미션
  FROM EMPLOYEES;
...
SELECT EMPLOYEE_ID
     ...
     , SALARY
     , DEPARTMENT_ID
     , CEIL(MONTHS_BETWEEN(SYSDATE, TO_DATE(HIRE_DATE, 'YY/MM/DD')))
  FROM EMPLOYEES;
