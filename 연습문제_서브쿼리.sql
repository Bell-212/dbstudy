...
SELECT EMP_NO, HIRE_DATE,
  ...
  FROM EMPLOYEE_T
 WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) >= (SELECT AVG(TRUNC(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)))
                                                       FROM EMPLOYEE_T);
                                            
-- 6. ''부서번호가 2인 부서에 근무하는 사원들의 직급'과 일치하는 사원'을 조회하시오.
SELECT *
  FROM EMPLOYEE_T
...
SELECT EMP_NO, NAME,HIRE_DATE
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE DESC) AS RN, EMP_NO, NAME,HIRE_DATE
         FROM EMPLOYEE_T)
