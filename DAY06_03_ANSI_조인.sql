/*
    ...
    2. 각 테이블의 관계(1:M) 이용해서 조인 조건을 만든다.
    3. 조인의 종류
        1) 내부 조인(INNER JOIN) : 2개 테이블에 모두 존재하는 테이터만 조회하는 방식
...
SELECT EMPLOYEE_ID
     ...
     , D.DEPARTMENT_ID
     , DEPARTMENT_NAME
  FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E -- 왼쪽 테이블(DEPARTMENTS)의 모든 데이터를 조회하시오. (사원이 존재하지 않는 부서도 조회하시오.)
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
...
-- 6. 부서번호, 부서명, 근무지역도시, 근무지역국가(부서, 근무지역, 국가 테이블 조인)
