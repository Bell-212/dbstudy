...
SELECT *
 ...
 SELECT *
  ...
  --WHERE SALARY >=10000 AND SALARY <=20000; 역시 같은 결과
 
 --6. 사원 테이블에서 부서번호가 30, 40, 50인 사원의 정보 조회하기
 SELECT *
   FROM EMPLOYEES
  WHERE DEPARTMENT_ID IN(30, 40, 50); -- 추천
  --WHERE DEPARTMENT_ID = 30 OR DEPARTMENT_ID = 40 OR DEPARTMENT_ID = 50; 같은 결과
  
 --7. 사원 테이블에서 부서번호가 없는 사원의 정보 조회하기
 SELECT *
   FROM EMPLOYEES
  WHERE DEPARTMENT_ID IS NULL; --WHERE DEPARTMENT_ID = NULL; 은 잘못된 절
...
SELECT *
  FROM EMPLOYEES
 WHERE PHONE_NUMBER LIKE '515' || '%'; --'515'가 사용자입장의 변수 부분이라고 가정할 때 용이 || 연결 연산자 사용
