...
SELECT TO_CHAR(1234)
     ...
     , TO_CHAR(12345, '99,999') -- '12,345' -- 숫자와 같은 자릿수로 형식도 지정 필요함.
  FROM DUAL;
...
SELECT SYSDATE       -- '23/07/04'
     , SYSTIMESTAMP  -- '23/07/04 14:47:04.145000000 +09:00'
  FROM DUAL;
  
-- 현재 날짜와 시간 - 형식 지정
SELECT TO_CHAR(SYSDATE,      'YYYY-MM-DD AM HH:MI:SS') --SYSDATE의 기본값은 날짜만 표시
...
-- DT1이 '23/07/04'인 데이터를 조회하기(조회되지 않는다.)
SELECT * 
  FROM EXAMPLE_T
 WHERE DT1 = '23/07/04';
...
-- DT1이 '23/07/04'인 데이터를 조회하기(조회된다.)
SELECT *
  FROM EXAMPLE_T
